...
public sealed class WorkforceLifecycleController(CareDbContext db, ITenantContext tenant, ICurrentUserContext user) : ControllerBase
{
    ...
    public async Task<IActionResult> Workspace(Guid workerId, CancellationToken token)
    {
        ...
        var employment = await One("select revision,recruitment_status,employment_status,contract_type,job_title,contracted_weekly_minutes,start_date,end_date,line_manager,status_reason,approved_by,approved_at,updated_at from worker_employment_profiles where care_worker_id=@worker and organization_id=@organization and (@wide or branch_id=@branch)", workerId, token);
    ...
    public Task<IActionResult> Employment(Guid workerId, EmploymentRequest request, CancellationToken token)
        => db.Database.CreateExecutionStrategy().ExecuteAsync(async () => {
            await using var tx = await db.Database.BeginTransactionAsync(token);
            var result = await EmploymentCore(workerId, request, token);
            await tx.CommitAsync(token);
            return result;
        });

    private async Task<IActionResult> EmploymentCore(Guid workerId, EmploymentRequest request, CancellationToken token)
    {
        ...
        if (request.EndDate < request.StartDate) return BadRequest(new { message = "End date cannot precede start date." });
        if (request.ExpectedRevision is null or < 0 || string.IsNullOrWhiteSpace(request.ChangeReason))
            return BadRequest(new { message = "Expected revision and a change reason are required. Use revision 0 for a new profile." });
        // Lock the parent worker too: a new employment profile has no row to lock yet.
        await using (var command = await Command("select \"BranchId\" from \"CareWorkers\" where \"Id\"=@worker and \"OrganizationId\"=@organization and (@wide or \"BranchId\"=@branch) for update",workerId,token))
        {
            if (await command.ExecuteScalarAsync(token) is not Guid) return NotFound();
        }
        var current = await One("select revision from worker_employment_profiles where care_worker_id=@worker and organization_id=@organization",workerId,token);
        var revision = current.TryGetValue("revision",out var value) ? Convert.ToInt32(value) : 0;
        if (request.ExpectedRevision != revision) return Conflict(new { message = "Employment profile changed. Reload it before saving.", currentRevision = revision });
        await Exec("""
            insert into worker_employment_profiles(care_worker_id,organization_id,branch_id,recruitment_status,employment_status,contract_type,job_title,contracted_weekly_minutes,start_date,end_date,line_manager,status_reason,approved_by,approved_at,updated_at,revision)
            select @worker,@organization,"BranchId",@recruitment,@status,@contract,@job,@minutes,@start,@end,@manager,@reason,@actor,now(),now(),1
            from "CareWorkers" where "Id"=@worker and "OrganizationId"=@organization
            on conflict(care_worker_id) do update set recruitment_status=@recruitment,employment_status=@status,
            contract_type=@contract,job_title=@job,contracted_weekly_minutes=@minutes,start_date=@start,end_date=@end,
            line_manager=@manager,status_reason=@reason,approved_by=@actor,approved_at=now(),updated_at=now(),revision=worker_employment_profiles.revision+1
            """,workerId,c=>{Add(c,"recruitment",request.RecruitmentStatus);Add(c,"status",request.EmploymentStatus);Add(c,"contract",request.ContractType);Add(c,"job",request.JobTitle);Add(c,"minutes",request.ContractedWeeklyMinutes);Add(c,"start",request.StartDate);Add(c,"end",request.EndDate);Add(c,"manager",request.LineManager);Add(c,"reason",request.StatusReason);},token);
        await Exec("""
            insert into worker_employment_history(id,care_worker_id,organization_id,branch_id,revision,snapshot,change_reason,actor_user_id,actor)
            select @id,care_worker_id,organization_id,branch_id,revision,to_jsonb(p),@reason,@actorId,@actor
            from worker_employment_profiles p where care_worker_id=@worker and organization_id=@organization;
            insert into workforce_lifecycle_events(id,care_worker_id,organization_id,branch_id,event_type,entity_type,entity_id,actor,detail)
            select @event,care_worker_id,organization_id,branch_id,'employment.updated','WorkerEmployment',care_worker_id,@actor,@reason
            from worker_employment_profiles where care_worker_id=@worker and organization_id=@organization;
            insert into "AuditEvents"("Id","Action","Actor","EntityType","EntityId","CreatedAt","OrganizationId","BranchId")
            select @audit,'employment.updated',@actor,'WorkerEmployment',care_worker_id,now(),organization_id,branch_id
            from worker_employment_profiles where care_worker_id=@worker and organization_id=@organization;
            """,workerId,c=>{Add(c,"id",Guid.NewGuid());Add(c,"event",Guid.NewGuid());Add(c,"audit",Guid.NewGuid());Add(c,"reason",request.ChangeReason.Trim());Add(c,"actorId",user.UserId);},token);
        return Ok(new { message = "Employment lifecycle updated.", revision = revision + 1 });
    }

    [HttpGet("employment/history")]
    [Authorize(Roles = "CareManager,Administrator")]
    public async Task<IActionResult> EmploymentHistory(Guid workerId,CancellationToken token)
    {
        if (!await WorkerExists(workerId,token)) return NotFound();
        var rows = await Many("select revision,snapshot::text,change_reason,actor_user_id,actor,recorded_at from worker_employment_history where care_worker_id=@worker and organization_id=@organization and (@wide or branch_id=@branch) order by revision desc",workerId,token);
        foreach (var row in rows) row["snapshot"] = System.Text.Json.JsonSerializer.Deserialize<System.Text.Json.JsonElement>((string)row["snapshot"]!);
        return Ok(rows);
    }
...
public sealed record EmploymentRequest(string RecruitmentStatus,string EmploymentStatus,string ContractType,string JobTitle,int ContractedWeeklyMinutes,DateOnly? StartDate,DateOnly? EndDate,string LineManager,string StatusReason,int? ExpectedRevision=null,string? ChangeReason=null);
