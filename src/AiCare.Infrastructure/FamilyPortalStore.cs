...
public sealed class FamilyPortalStore : IFamilyPortalStore
{
    ...
    public async Task<FamilyAccessSnapshot?> GetAccessAsync(Guid organizationId, Guid familyMemberId, CancellationToken cancellationToken)
    {
        ...
        if (row is null)
        {
            return new FamilyAccessSnapshot(member.Id, member.ServiceUserId, member.FullName, member.Email, member.Relationship,
                member.Relationship, "Pending", "PendingVerification", null, null, 0, Array.Empty<string>(),
                invitation.Status, invitation.ExpiresAt);
        }

        var permissions = await ReadPermissionsAsync(row.Value.Id, cancellationToken);
        return new FamilyAccessSnapshot(member.Id, member.ServiceUserId, member.FullName, member.Email, member.Relationship,
            row.Value.AuthorityType, row.Value.VerificationStatus, EffectiveStatus(row.Value.AccessStatus, row.Value.ValidFrom, row.Value.ValidUntil),
    ...
    {
        var member = await _db.FamilyMembers.AsNoTracking().FirstOrDefaultAsync(x => x.OrganizationId == organizationId && x.Id == command.FamilyMemberId, cancellationToken)
        ...
        await _db.Database.ExecuteSqlInterpolatedAsync($"""
            ...
            values ({grantId}, {member.Id}, {member.ServiceUserId}, {command.AuthorityType}, {command.VerificationStatus}, {accessStatus},
                {actorUserId}, {actorName}, {(verified ? DateTimeOffset.UtcNow : (DateTimeOffset?)null)},
                {command.ValidFrom}, {command.ValidUntil}, {nextRevision}, now(), now(), {organizationId}, {branchId})
    ...
    {
        var member = await _db.FamilyMembers.AsNoTracking().FirstOrDefaultAsync(x => x.OrganizationId == organizationId && x.Id == familyMemberId, cancellationToken)
            ?? throw new InvalidOperationException("Family member was not found.");
        await _db.Database.ExecuteSqlInterpolatedAsync($"""
            update family_portal_invitations
        ...
        await _db.Database.ExecuteSqlInterpolatedAsync($"""
            ...
            values ({id}, {familyMemberId}, {tokenHash}, {member.Email}, {'S' + "ent"}, {expiresAt}, now(), {actorUserId}, {actorName}, {organizationId}, {branchId})
            """, cancellationToken);
        AddAudit(organizationId, branchId, actorName, "family.invited", "FamilyMember", familyMemberId);
        await _db.SaveChangesAsync(cancellationToken);
        return new FamilyInvitationResult(id, "Sent", expiresAt, null);
    }
    ...
    public async Task<FamilyInvitationValidation> ValidateInvitationAsync(string tokenHash, DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        if (expiresAt <= now) return new(false, "Expired", "This invitation has expired. Please ask the care provider to send a new invitation.", fullName, providerName, expiresAt);
        if (status is not ("Pending" or "Sent")) return new(false, status, "This invitation has already been used or revoked.", fullName, providerName, expiresAt);
    ...
    public async Task AcceptInvitationAsync(string tokenHash, string password, DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        {
            var user = new AppUser(Guid.NewGuid(), email, email, passwordHash, UserRole.FamilyMember, true, organizationId, branchId, null, familyMemberId);
    ...
    public async Task SetAccessStatusAsync(Guid organizationId, Guid actorUserId, string actorName, Guid familyMemberId, string status, CancellationToken cancellationToken)
    {
        ...
        var changed = verificationStatus is null
            ? await _db.Database.ExecuteSqlInterpolatedAsync($"update family_access_grants set access_status = {accessStatus}, revision = revision + 1, updated_at = now() where organization_id = {organizationId} and family_member_id = {familyMemberId}", cancellationToken)
            : await _db.Database.ExecuteSqlInterpolatedAsync($"update family_access_grants set access_status = {accessStatus}, verification_status = {verificationStatus}, revision = revision + 1, updated_at = now() where organization_id = {organizationId} and family_member_id = {familyMemberId}", cancellationToken);
    ...
    public async Task<bool> HasPermissionAsync(Guid organizationId, Guid familyMemberId, Guid serviceUserId, string permission, DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        if (EffectiveStatus(grant.Value.AccessStatus, grant.Value.ValidFrom, grant.Value.ValidUntil) != "Active") return false;
    ...
    public async Task<FamilyFeedbackResult> SubmitFeedbackAsync(Guid organizationId, Guid? branchId, Guid familyMemberId, FamilyFeedbackInput input, CancellationToken cancellationToken)
    {
        ...
        await _db.Database.ExecuteSqlInterpolatedAsync($"""
            ...
            values ({id}, {input.ServiceUserId}, {familyMemberId}, {input.Type}, {input.Subject}, {input.Description}, {input.Priority}, {'S' + "ubmitted"}, {submittedAt}, {submittedAt.AddDays(input.Type == "Complaint" ? 2 : 5)}, '', {organizationId}, {branchId})
            """, cancellationToken);
        AddAudit(organizationId, branchId, familyMemberId.ToString(), "family.feedback_submitted", "FamilyFeedbackCase", id);
        await _db.SaveChangesAsync(cancellationToken);
        return new FamilyFeedbackResult(id, "Submitted", submittedAt);
    }
    ...
    private async Task<(Guid Id, Guid ServiceUserId, string AuthorityType, string VerificationStatus, string AccessStatus, DateTimeOffset? ValidFrom, DateTimeOffset? ValidUntil, long Revision)?> ReadGrantAsync(Guid organizationId, Guid familyMemberId, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            ...
            where organization_id = @organization and family_member_id = @family
            limit 1
            """;
        ...
        return (reader.GetGuid(0), reader.GetGuid(1), reader.GetString(2), reader.GetString(3), reader.GetString(4),
            reader.IsDBNull(5) ? null : reader.GetFieldValue<DateTimeOffset>(5), reader.IsDBNull(6) ? null : reader.GetFieldValue<DateTimeOffset>(6), reader.GetInt64(7));
    }
    ...
    private static string EffectiveStatus(string accessStatus, DateTimeOffset? validFrom, DateTimeOffset? validUntil)
    {
