...
{
 [Fact]
 public async Task PublicWebhookIsBoundedRequiresFreshValidSignatureAndIsIdempotent()
 {
  ...
  var oversized="{\"value\":\""+new string('x',256*1024)+"\"}";Assert.Equal(HttpStatusCode.RequestEntityTooLarge,(await admin.SendAsync(Message(connector.Id,oversized,timestamp,$"large-{Guid.NewGuid():N}",new string('0',64)))).StatusCode);
  var arrayBody="[]";var arraySignature=Sign(connector.WebhookSecret,timestamp,arrayBody);Assert.Equal(HttpStatusCode.BadRequest,(await admin.SendAsync(Message(connector.Id,arrayBody,timestamp,$"array-{Guid.NewGuid():N}",arraySignature))).StatusCode);
  var invalid=Message(connector.Id,body,timestamp,eventId,new string('0',64));Assert.Equal(HttpStatusCode.Unauthorized,(await admin.SendAsync(invalid)).StatusCode);
  var signature=Sign(connector.WebhookSecret,timestamp,body);
 ...
 private static string Sign(string secret,string timestamp,string body)=>Convert.ToHexString(HMACSHA256.HashData(Encoding.UTF8.GetBytes(secret),Encoding.UTF8.GetBytes($"{timestamp}.{body}"))).ToLowerInvariant();
